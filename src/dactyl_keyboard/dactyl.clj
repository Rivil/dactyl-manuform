...
(def handwire-hotswap
  (let [spacer (->> (difference (->> (cube (+ keyswitch-width 3) (+ keyswitch-height 3) 3)
                                     (translate [0 0 -2]))
                                (->> (cube (+ keyswitch-width 0) (+ keyswitch-height 0) 4)
                                     (translate [0 0 -2]))))
        bottom-holder (->> (difference (->> (cube (+ keyswitch-height 2) (+ keyswitch-width 2) 1)
                                            (translate [0 0 -4]))
                                       (->> (cube (- keyswitch-height 3) (- keyswitch-width 3) 2)
                                            (translate [0 0 -4]))))
        cable-cross (->> (union (->> (cube (+ keyswitch-width 3) 0.7 0.8)
                                     (translate [0 0 -4.15]))
                                (->> (cube 0.7 (+ keyswitch-height 3) 0.8)
                                     (translate [0 0 -4.15]))))]
    (union spacer
