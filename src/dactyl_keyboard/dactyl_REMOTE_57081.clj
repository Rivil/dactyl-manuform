...
(def kaylih-hotswap-plate
  (let [spacer (->>    (difference (->> (cube (+ keyswitch-width 2) (+ keyswitch-height 2) 0.5)
                                        (translate [0 0 -0.75]))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Space for handwired hotswap ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(def handwire-hotswap
  (let [spacer (->> (difference (->> (cube (+ keyswitch-width 1) (+ keyswitch-height 1) 3)
                                     (translate [0 0 -2]))
                                (->> (cube (+ keyswitch-width 0) (+ keyswitch-height 0) 4)
                                     (translate [0 0 -2]))))
        bottom-holder (->> (difference (->> (cube (+ keyswitch-height 1) (+ keyswitch-width 1) 0.6)
                                            (translate [0 0 -3.75]))
                                       (->> (cube (- keyswitch-height 3) (- keyswitch-width 3) 2)
                                            (translate [0 0 -3.75]))))
        cable-cross (->> (union (->> (cube (+ keyswitch-width 2) 0.7 0.7)
                                     (translate [0 0 -3.75]))
                                (->> (cube 0.7 (+ keyswitch-height 2) 0.7)
                                     (translate [0 0 -3.75]))))]
    (union spacer
           (difference bottom-holder cable-cross))))
  ...
  (let [top-wall (->> (cube (+ keyswitch-width 3) 1.5 (+ plate-thickness 0.5))
        ...
        top-nub-pair (union top-nub
                            (->> top-nub
                                 ...
                                 (mirror [0 1 0])))]
    (difference
     (union handwire-hotswap plate-half
            (->> plate-half
                 (mirror [1 0 0])
                 (mirror [0 1 0])))
     (->>
